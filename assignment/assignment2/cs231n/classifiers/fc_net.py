...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            if i == 0:
                self.params['W1'] = np.random.normal(loc=0, scale=weight_scale, size=(input_dim, hidden_dims[i]))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(num_hidden):
            ...
            hidden_in_val.append(hidden_in)
            ...
            if self.normalization is not None:
                ...
                if self.normalization == 'layernorm':
                    hidden_norm, cache_ln = layernorm_forward(hidden_in, self.params[str_gamma], self.params[str_beta], self.bn_params[i])
        ...
        scores = hidden_out @ self.params[str_W] + self.params[str_b]
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            str_gamma = 'gamma' + str(i)
            str_beta = 'beta' + str(i)

            if self.normalization is not None:
                if self.normalization == 'batchnorm':
                    grad_x_out, grads[str_gamma], grads[str_beta] = \
                        batchnorm_backward_alt(grad_x_norm, cache_bn_value[i - 1])
                if self.normalization == 'layernorm':
                    grad_x_out, grads[str_gamma], grads[str_beta] = \
