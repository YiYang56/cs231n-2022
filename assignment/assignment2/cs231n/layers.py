...
def svm_loss(x, y):
    ...
    N = x.shape[0]
    correct = x[range(N), y]
    margin = np.maximum(x - correct[:, np.newaxis] + 1, 0)
    margin[range(N), y] = 0
      ...
      for j in range(F):
        for hh in range(H_out):
          for ww in range(W_out):
            out[i][j][hh][ww] = np.sum(x_tmp[:, stride * hh : stride * hh + HH, stride * ww : stride * ww + WW] * w[j]) + b[j]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
      for j in range(C):
        for hh in range(H_out):
          for ww in range(W_out):
            pool = x[i, j, hh * stride : hh * stride + pool_height, ww * stride : ww * stride + pool_width]
            mask = np.zeros(pool.shape)
            mask[pool == np.max(pool)] = 1
            num = np.sum(mask)
            mask /= num
            dx[i, j, hh * stride : hh * stride + pool_height, ww * stride : ww * stride + pool_width] = \
              mask * dout[i][j][hh][ww]
